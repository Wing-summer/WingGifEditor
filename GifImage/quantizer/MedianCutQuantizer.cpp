...
struct Cluster {
  ...
  bool split(std::vector<ARGB> &pixels, Cluster *cluster1, Cluster *cluster2) {
    ...
    std::sort(pixels.begin() + start, pixels.begin() + end,
              cmp[componentWithLargestSpread]);
...
int32_t MedianCutQuantizer::quantize(const std::vector<ARGB> &in,
                                     uint32_t maxColorCount,
                                     std::vector<ARGB> &out) {
  size_t pixelCount = in.size();
  std::priority_queue<Cluster> clusters;
  Cluster cluster;
  cluster.setStartAndEnd(0, int(pixelCount));
  ...
  for (int i = 0; i < resultSize; i++) {
    ...
    for (int j = top.start; j < top.end; j++) {
      sumR += sortPixels[ulong(j)].r;
      sumG += sortPixels[ulong(j)].g;
      sumB += sortPixels[ulong(j)].b;
    }
    auto r = static_cast<uint8_t>(sumR / uint(top.pixelSize));
    auto g = static_cast<uint8_t>(sumG / uint(top.pixelSize));
    auto b = static_cast<uint8_t>(sumB / uint(top.pixelSize));
