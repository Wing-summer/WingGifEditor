...
bool OctreeQuantizer::addColor(Node **node, uint32_t r, uint32_t g, uint32_t b,
                               int level) {
  if (*node == nullptr) {
  ...
  if ((*node)->isLeaf) {
    ...
    int shift = 7 - level;
    auto index = (((r & mask[level]) >> shift) << 2) |
...
void OctreeQuantizer::getColorPalette(Node *tree, int32_t &inIndex,
                                      std::vector<ARGB> &out) {
  if (tree == nullptr) {
    return;
  }
...
int32_t OctreeQuantizer::quantize(const std::vector<ARGB> &in,
...
void OctreeQuantizer::getColorIndices(const std::vector<ARGB> &pixels,
                                      uint8_t *out) {
