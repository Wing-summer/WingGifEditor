...
int32_t KMeansQuantizer::quantize(const std::vector<ARGB> &in,
  ...
  std::uniform_int_distribution<uint32_t> dis(0, uint32_t(pixelCount));
  ...
  while (loop) {
    ...
    // classification
    {
      ...
      kdTree.createKDTree(&rootNode, datas, 0, int(maxColorCount - 1), 0);
