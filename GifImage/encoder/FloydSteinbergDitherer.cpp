...
static const float ERROR_COMPONENT_FACTION[] = {7.0f / 16.0f, 3.0f / 16.0f,
                                                5.0f / 16.0f, 1.0f / 16.0f};

void FloydSteinbergDitherer::dither(std::vector<ARGB> &origin,
                                    std::vector<ARGB> &quantize,
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      ...
      auto errorR = int8_t(r - nearestCentroidR);
      auto errorG = int8_t(g - nearestCentroidG);
      auto errorB = int8_t(b - nearestCentroidB);

      for (int directionId = 0; directionId < ERROR_COMPONENT_SIZE;
           ++directionId) {
        auto siblingX =
            static_cast<int32_t>(x + ERROR_COMPONENT_DELTA_X[directionId]);
        auto siblingY =
            static_cast<int32_t>(y + ERROR_COMPONENT_DELTA_Y[directionId]);

        if (siblingX >= 0 && siblingY >= 0 && siblingX < width &&
            siblingY < height) {
          ...
          auto ditherPosition = ulong(siblingY * width + siblingX);
          ...
          origin[ditherPosition].r =
              static_cast<uint8_t>(std::min(255, std::max(0, siblingR)));
          origin[ditherPosition].g =
              static_cast<uint8_t>(std::min(255, std::max(0, siblingG)));
          origin[ditherPosition].b =
              static_cast<uint8_t>(std::min(255, std::max(0, siblingB)));
        }
      }
    }
  }
  for (int i = 0; i < totalSize; ++i) {
    auto rgb = origin[ulong(i)];
